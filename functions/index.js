...
const LINE_HEADER = {
  ...
  'Authorization': 'Bearer '+ '<Channel Access Token' // Channel access token (long-lived) 
};
...
exports.ThaiPostTrackBot = functions.region(REGION).https.onRequest((req, res) => {
    ...
    async function settrack(req, code) 
    {
        let promise_token = new Promise(resolve => {
            var options = {
                ...
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': 'Token ' + '<Thai post Token>' // <Thai post Token>
                }
            };
    ...
    async function gettrack(req, message)
    {
        ...
        if (items[key[0]].length > 0) {
            ...
            reply_text(req, 'ไม่พบหมายเลขพัสดุที่ระบุ | Ex: EA473124280TH');
        }
    }
});
...
const push_update_status = (req) => {
    return request({
        ...
        headers: LINE_HEADER,
        body: JSON.stringify({
            to: '<Your user ID>', // Your user ID Line OA Bot
            messages: [
                {
                    type: `text`,
